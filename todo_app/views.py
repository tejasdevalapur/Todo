    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                return render(request, 'todo_app/signupuser.html', {'form':UserCreationForm(), 'error':'That username has already been taken. Please choose a new username'})
        else:
...
def viewtodo(request,todo_pk):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'todo_app/viewtodo.html',{'todo':todo,'form':form,'error':'Bad data..Try again'})
@login_required
     ...
     if request.method == 'GET':
        return render(request, 'todo_app/createtodo.html', {'form':TodoForm()})
         ...
         try:
            ...
            return render(request, 'todo_app/createtodo.html', {'form':TodoForm(),'error':'Entered Data is invalid..Try Again'})
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
             return render(request, 'todo_app/loginuser.html', {'form':AuthenticationForm(),'error':'Username and Password does not exists'})
        else:
            login(request, user)
            return redirect('currenttodos')
